		...
		jQuery.ajax({
			...
			success: function(data){
				...
				jQuery.each(records, function(index, record){
					...
					netmemberdirectory_spread_points(pointsArr, record);
...
/*
 * If two points are at the same location, spread them out
 */
function netmemberdirectory_spread_points(pointsArr, record){

	var centerLat = record["latitude"];
	var centerLon = record["longitude"];

	if ((typeof pointsArr[centerLat] != "undefined" || pointsArr[centerLat] != null) &&
		(typeof pointsArr[centerLat][centerLon] != "undefined" || pointsArr[centerLat][centerLon] != null)){
		// a point exists at this location need to update

		var radius = 0.005;
		var degree = 90;
		var foundEmptySpace = false

		for(degree; degree < 360; degree+= 10){
			if((typeof pointsArr[centerLat] != "undefined" || pointsArr[centerLat] != null) &&
			(typeof pointsArr[centerLat][centerLon] != "undefined" || pointsArr[centerLat][centerLon] != null)){

				centerLat = (Math.cos(Math.PI/180.0 * degree) * radius) + parseFloat(record["latitude"]);
				centerLon = (Math.sin(Math.PI/180.0 * degree) * radius) + parseFloat(record["longitude"]);
		...
		if(foundEmptySpace == true){
			record["latitude"] = centerLat;
			record["longitude"] = centerLon;
		}

	}

	if (typeof pointsArr[record["latitude"]] == "undefined" || pointsArr[record["latitude"]] == null){
		pointsArr[record["latitude"]] = {};
	}
	pointsArr[record["latitude"]][record["longitude"]] = 1;
	return;
}
